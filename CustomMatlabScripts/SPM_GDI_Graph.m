function SPM_GDI_Graph(DataStruct, no_deviation_clouds, PlotTitles, PlotYLabels, PlotYLabelsSecondary, nParticipantsPlots, PlotXLabel, LegendText, colorScheme, lineWidth, errorAlpha)
    arguments
        DataStruct
...
HeightPartitions = timeSeriesHeightScaler*3 + subplotScaler*18 + subplotSpacingScaler*5 + topSpacingScaler + botSpacingScaler; %The height of the 21 rows + spacing
...
for i = 1:3 % number of mean plot rows
    ...
    for p = 1:3 % number of mean plot collumns
        ...
        for o = 1:6 % number of subplot rows for each mean plot
    ...
    SubplotHeightCount = SubplotHeightCount-6;
...
for k = 1:numFeatures
    ...
    text(1,ypos,PlotTitles(k),'HorizontalAlignment','left','VerticalAlignment','bottom')
    ...
    for i = 1:6
        ...
        if ismember(k,[1,2,3,4,5,6]) || ismember(i, [1,2,3,4,5])
            set(gca, 'XTick', []);
            set(gca, 'XTickLabel', []);
            xlabel([])
        end
