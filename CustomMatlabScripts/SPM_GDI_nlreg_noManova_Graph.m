function SPM_GDI_nlreg_noManova_Graph(DataStruct, no_deviation_clouds, PlotTitles, PlotYLabels, PlotXLabel, LegendText, colorScheme, lineWidth, errorAlpha)
    arguments
        DataStruct
...
HeightPartitions = timeSeriesHeightScaler*3 + subplotScaler*9 + subplotSpacingScaler*5 + topSpacingScaler + botSpacingScaler; %The height of the 15 rows + spacing
...
for i = 1:3 % number of mean plot rows
    ...
    SubplotHeightCount = SubplotHeightCount-3;
...
for k = 1:numFeatures
    ...
    for i = 1:3
        ...
        if ismember(k,[1,2,3,4,5,6]) || ismember(i, [1,2])
            set(gca, 'XTick', []);
            set(gca, 'XTickLabel', []);
            xlabel([])
        end
